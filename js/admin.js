...
(function () {
  ...
  const REPONSES = {
    fr: { confirmee: "Bonjour {nom}, votre commande n°{n} est confirmée ({creneau}). À très vite ! — Bianchi Dessert", prete: "Bonjour {nom}, votre commande n°{n} est prête 🍰 — Bianchi Dessert", relance: "Bonjour {nom}, votre commande n°{n} est réservée sur notre site. Souhaitez-vous la confirmer ? — Bianchi Dessert" },
    ar: { confirmee: "مرحباً {nom}، تم تأكيد طلبك رقم {n} ({creneau}). إلى اللقاء قريباً! — Bianchi Dessert", prete: "مرحباً {nom}، طلبك رقم {n} جاهز 🍰 — Bianchi Dessert", relance: "مرحباً {nom}، طلبك رقم {n} محجوز على موقعنا. هل تريد تأكيده؟ — Bianchi Dessert" },
    en: { confirmee: "Hello {nom}, your order no. {n} is confirmed ({creneau}). See you soon! — Bianchi Dessert", prete: "Hello {nom}, your order no. {n} is ready 🍰 — Bianchi Dessert", relance: "Hello {nom}, your order no. {n} is reserved on our website. Would you like to confirm it? — Bianchi Dessert" },
    de: { confirmee: "Hallo {nom}, Ihre Bestellung Nr. {n} ist bestätigt ({creneau}). Bis bald! — Bianchi Dessert", prete: "Hallo {nom}, Ihre Bestellung Nr. {n} ist fertig 🍰 — Bianchi Dessert", relance: "Hallo {nom}, Ihre Bestellung Nr. {n} ist auf unserer Website reserviert. Möchten Sie sie bestätigen? — Bianchi Dessert" },
    nl: { confirmee: "Hallo {nom}, uw bestelling nr. {n} is bevestigd ({creneau}). Tot snel! — Bianchi Dessert", prete: "Hallo {nom}, uw bestelling nr. {n} is klaar 🍰 — Bianchi Dessert", relance: "Hallo {nom}, uw bestelling nr. {n} is gereserveerd op onze website. Wilt u deze bevestigen? — Bianchi Dessert" },
  };
  ...
  function reponseWa(c, type) {
    ...
    const txt = REPONSES[l][type].replace("{nom}", c.client_nom).replace("{n}", c.numero).replace("{creneau}", cren);
  ...
  async function initLogin() {
    if (api.mode === "demo") { $("#emailField").hidden = true; $("#passLabel").textContent = "Code d'accès"; $("#demoHint").hidden = false; }
  ...
  function commandeCard(c) {
    ...
    const actions = c.statut === "en_attente" ? `<a class="btn btn-wa btn-sm" data-rep="confirmee" data-cmd-id="${c.id}" href="${reponseWa(c, "confirmee")}" target="_blank" rel="noopener">✅ Confirmer sur WhatsApp</a>${nonEnvoye ? `<a class="btn btn-ghost btn-sm" href="${reponseWa(c, "relance")}" target="_blank" rel="noopener">🔔 Relancer le client</a>` : ""}` : c.statut === "confirmee" ? `<a class="btn btn-wa btn-sm" data-rep="prete" data-cmd-id="${c.id}" href="${reponseWa(c, "prete")}" target="_blank" rel="noopener">🍰 Prévenir : c'est prêt</a>` : "";
    return `<div class="order${location.hash === "#cmd=" + c.numero ? " hl" : ""}" id="cmd-${c.numero}">
      <div class="order-head"><b>Commande n°${c.numero}</b><span class="when">${d.toLocaleDateString("fr-FR")} ${d.toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" })}</span>${nonEnvoye ? `<span class="pill pill-warn" title="Le client n'a pas ouvert WhatsApp avec le récapitulatif">Récap non envoyé</span>` : ""}<span class="spacer"></span>
