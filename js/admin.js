...
(function () {
  ...
  const REPONSES = {
    fr: { confirmee: "Bonjour {nom}, votre commande n° {n} est confirmée ({creneau}). A très vite ! - Bianchi Dessert", prete: "Bonjour {nom}, votre commande n° {n} est prête. - Bianchi Dessert", relance: "Bonjour {nom}, votre commande n° {n} est réservée sur notre site. Souhaitez-vous la confirmer ? - Bianchi Dessert" },
  ...
  function reponseWa(c, type) {
    ...
    const txt = REPONSES[l][type].replace("{nom}", c.client_nom).replace("{n}", c.code || c.numero).replace("{creneau}", cren);
